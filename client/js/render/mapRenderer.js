export default class MapRenderer {
    ...
    drawWalls(x, y, id, offx, offy, obj) {
        ...
        if (obj=='objects') {
            ...
            objHeight = this.phaser.cache.getImage(obj).height;
            objWidth = this.phaser.cache.getImage(obj).width;
        }
        ...
        //this.phaser.spriteGroup.add(temp);
    }
            ...
            for (let j = 0; j < this.mapHeight; j++) {
                ...
                if(this.map.layers[3].data[tileCount] != 0) {
                    if(this.map.layers[3].data[tileCount] >= 1561) {
                        this.drawWalls(i, j, this.map.layers[3].data[tileCount] - 1561, -8, 0, 'objects');
                    }
                }

                if (this.map.layers[4].data[tileCount] != 0) {
                    if(this.map.layers[4].data[tileCount] >= 1561) {
                        this.drawWalls(i, j, this.map.layers[4].data[tileCount] - 1561, 0, 0, 'objects');
                    }
                }
